...
def parse_model(d, ch):
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in {
            ...
            if m is SE:
                c2 = c1
                reduction = args[0] if len(args) else 16
                if reduction <= 0:
                    raise ValueError(f"SE reduction must be > 0, got {reduction}")
                args = [c1, reduction]
            elif m is CBAM:
                c2 = c1
                reduction = args[0] if len(args) else 16
                spatial_kernel = args[1] if len(args) > 1 else 7
                if reduction <= 0:
                    raise ValueError(f"CBAM reduction must be > 0, got {reduction}")
                args = [c1, reduction, spatial_kernel]
            else:
                c2 = args[0]
                if c2 != no:  # if not output
                    c2 = make_divisible(c2 * gw, ch_mul)
