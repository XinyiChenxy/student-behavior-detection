...
class ComputeLoss:
    ...
    def __init__(self, model, autobalance=False):
        ...
        # Mild class-weighted BCE for sb_no_hr experiment
        # Use square-root inverse frequency to avoid over-suppressing head classes.
        class_counts = torch.tensor([94, 99, 782, 631, 329], dtype=torch.float32, device=device)
        class_weights = (class_counts.sum() / class_counts).pow(0.5)
        class_weights = class_weights / class_weights.mean()
        self.class_weights = class_weights.clamp(min=0.7, max=1.8)
        self.cls_pos_weight = torch.tensor([h["cls_pw"]], device=device)
        self.fl_gamma = h["fl_gamma"]
        self.fl_alpha = 0.25
            ...
            if n := b.shape[0]:
                ...
                if self.nc > 1:  # cls loss (only if multiple classes)
                    ...
                    if self.fl_gamma > 0:
                        ...
                        alpha_factor = t * self.fl_alpha + (1 - t) * (1 - self.fl_alpha)
                        modulating_factor = (1.0 - p_t) ** self.fl_gamma
                        cls_loss *= alpha_factor * modulating_factor
                    cls_loss *= self.class_weights.view(1, -1)
                    lcls += cls_loss.mean()  # weighted BCE
